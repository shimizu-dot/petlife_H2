...
・メッセージイベントに対して自動返信（症状キーワード案内、`SlackBotService.buildReply()`）  
・管理者 Slack チャンネルへの個別通知送信（`SlackBotService.postMessage(channel, text)`）  
・利用設定：`SLACK_BOT_TOKEN`、`SLACK_SIGNING_SECRET`、`ADMIN_SLACK_USER_IDS`  
・プラン条件：Standard 以上（`SLACK_BOT` feature）  
・**一斉送信（Broadcast）は未実装。** LINE の Broadcast API 相当機能（全フォロワーへの一斉配信）は存在しない。将来実装する場合は `SlackBotService.broadcastMessage()` を追加し管理画面 `/app/admin/slack/push` を新設すること。

### LINE Messaging API
・LINE Webhook 受信エンドポイント：`/api/line/events`  
・テキストメッセージに対して自動返信  
・管理者 LINE への通知送信対応  
・**全ユーザー一斉送信（Broadcast）実装済み。** `LinePushController`（`GET/POST /app/admin/line/push`）から `LineBotService.broadcastMessage()` 経由で Bot の全フォロワーへ配信。  
...
| SUPER | 開発者アカウント（全権限） | 2 |
| USER | 一般飼い主ユーザー（プラン別機能利用） | 3 |
| VET | 獣医師（診療記録・予約管理・ユーザー閲覧） | 4 |
| STAFF | スタッフ（診療・予約・ユーザー管理・運営管理） | 5 |

### スタッフ系ロール別アクセス権限

| 機能 | URL | 開発者 | 管理者 | 獣医師 | スタッフ | 備考 |
|---|---|:---:|:---:|:---:|:---:|---|
| 診療記録 | `/app/consultations/**` | ◎ | ✖ | ◎ | ◎ | ADMIN は閲覧不可 |
| 診療予約 管理 | `/app/appointments/**` | ◎ | ✖ | ◎ | ◎ | 承認・却下は SUPER/VET/STAFF のみ |
| カレンダー | `/app/calendar/**` | ◎ | ✖ | ◎ | ◎ | ADMIN は閲覧不可 |
| お知らせ管理 | `/app/admin/announcements/**` | ◎ | ◎ | ✖ | ◎ | VET は閲覧不可 |
| ユーザ管理 | `/app/admin/users/**` | ◎（フル） | ◎（フル） | △（一覧閲覧のみ） | ◎（作成・編集） | 削除は ADMIN/SUPER のみ |
| 予約枠管理 | `/app/admin/appointment-slots/**` | ◎ | ◎ | ✖ | ◎ | VET は閲覧不可 |
| サービス統計 | `/app/reports/**` | ◎ | ◎ | ✖ | ✖ | ADMIN/SUPER 専用 |
...
※ `SUPER` は全権限のため、下記すべての機能にアクセス可能

| URL | メソッド | 画面/機能 | ロール制限 |
|---|---|---|---|
| `/app/dashboard` | GET | ダッシュボード | 全ロール（SUPER 含む） |
| `/app/pets` | GET / POST | ペット一覧・登録 | 全ロール（SUPER 含む） |
| `/app/pets/{id}` | GET / PATCH / DELETE | ペット詳細・編集・削除 | 全ロール（SUPER 含む） |
| `/app/pets/{id}/health-records` | GET / POST | 健康記録 | 全ロール（SUPER 含む） |
| `/app/pets/{id}/health-records/{rid}` | PATCH / DELETE | 健康記録編集・削除 | 全ロール（SUPER 含む） |
| `/app/calendar` | GET | ペットカレンダー | SUPER / USER / VET / STAFF |
| `/app/calendar/marks/add` | POST | カレンダーシール追加 | SUPER / USER / VET / STAFF |
| `/app/calendar/marks/{id}/delete` | POST | シール削除 | SUPER / USER / VET / STAFF |
| `/app/appointments` | GET / POST | 診療予約一覧・新規作成 | SUPER / USER / VET / STAFF |
| `/app/appointments/{id}/approve` | POST | 予約承認 | SUPER / VET / STAFF |
| `/app/appointments/{id}/reject` | POST | 予約却下 | SUPER / VET / STAFF |
| `/app/appointments/{id}/cancel` | POST | 予約キャンセル | SUPER / USER |
| `/app/appointments/delete-selected` | POST | 選択削除 | SUPER / USER / VET / STAFF |
| `/app/consult/chatbot` | GET / POST | チャットボット相談 | 全ロール（認証済み） |
| `/app/clinic-guide` | GET | 動物病院案内 | 全ロール（SUPER 含む） |
| `/app/notifications` | GET | 通知・リマインダー一覧 | 全ロール（SUPER 含む） |
| `/app/notifications/{id}/read` | POST | 既読 | 全ロール（SUPER 含む） |
| `/app/notifications/read-all` | POST | 全既読 | 全ロール（SUPER 含む） |
| `/app/notifications/reminders/dismiss` | POST | リマインダー非表示 | 全ロール（SUPER 含む） |
| `/app/subscriptions` | GET | サブスクリプション確認 | 全ロール（SUPER 含む / ADMIN は全件） |
| `/app/password-resets` | GET / POST | パスワード変更（要現在PW） | 全ロール（SUPER 含む） |
| `/app/premium/online-care` | GET / POST | Zoom オンライン診療予約 | SUPER / Premium のみ |

### 診療・予約管理（SUPER / VET / STAFF）
...
### 管理者・SUPER・スタッフ（`/app/admin/**`）

| URL | メソッド | アクセス可能ロール | 画面/機能 |
|---|---|---|---|
| `/app/admin/users` | GET | SUPER / ADMIN / VET / STAFF | ユーザー一覧 |
| `/app/admin/users/new` | GET / POST | SUPER / ADMIN / STAFF | ユーザー新規登録 |
| `/app/admin/users/{id}/edit` | GET / PATCH | SUPER / ADMIN / STAFF | ユーザー編集 |
| `/app/admin/users/{id}` | DELETE | SUPER / ADMIN | ユーザー削除 |
| `/app/admin/announcements` | GET / POST | SUPER / ADMIN / STAFF | お知らせ管理 |
| `/app/admin/announcements/{id}/toggle` | POST | SUPER / ADMIN / STAFF | 公開・非公開切替 |
| `/app/admin/announcements/{id}/delete` | POST | SUPER / ADMIN / STAFF | お知らせ削除 |
| `/app/admin/appointment-slots` | GET / POST | SUPER / ADMIN / STAFF | 予約枠管理 |
| `/app/admin/appointment-slots/{id}/delete` | POST | SUPER / ADMIN / STAFF | 予約枠削除 |
| `/app/admin/billing` | GET | SUPER / ADMIN | 請求一覧 |
| `/app/admin/billing/{id}` | GET | SUPER / ADMIN | 請求詳細・入金登録フォーム |
| `/app/admin/billing/{id}/pay` | POST | SUPER / ADMIN | 入金登録 |
| `/app/admin/billing/{id}/resend-notification` | POST | SUPER / ADMIN | お支払い案内を顧客へ再送（アプリ内通知＋メール＋LINE） |
| `/app/admin/line/push` | GET / POST | SUPER / ADMIN | LINE 一斉配信管理 |
...
| `/app/invoices/{id}` | GET | 請求書・領収書ビュー（ブラウザ印刷対応、`UserInvoiceController`） |
...
| `ConsultationController` | `/app/consultations` | 診療記録 CRUD（SUPER/VET/STAFF のみ） |
| `ConsultChatController` | `/app/consult/chatbot` | チャットボット相談（全プラン利用可） |
...
| `UserController` | `/app/admin/users` | ユーザー管理（SUPER/ADMIN フル / STAFF 作成・編集 / VET 閲覧） |
| `AnnouncementController` | `/app/admin/announcements` | お知らせ管理（SUPER / ADMIN / STAFF） |
| `AppointmentSlotController` | `/app/admin/appointment-slots` | 予約枠管理（SUPER / ADMIN / STAFF） |
| `ReportController` | `/app/reports` | サービス統計（ADMIN/SUPER のみ） |
| `BillingController` | `/app/admin/billing` | 請求・決済管理（ADMIN/SUPER のみ） |
| `UserInvoiceController` | `/app/invoices/{id}` | 請求書・領収書ビュー |
| `LinePushController` | `/app/admin/line/push` | LINE 一斉配信管理（ADMIN/SUPER のみ） |
| `DatabaseBackupController` | `/app/admin/database` | DB バックアップ管理（SUPER のみ） |
| `AccessDeniedPageController` | `/app/access-denied` | アクセス拒否ページ |
...
# OpenAI（AI症状チェック F-009）※未設定でもキーワードベースフォールバック動作
openai.api-key=sk-...
# openai.model=gpt-4.1-mini  # デフォルト

# Chatbot（チャットボット相談 F-022 / Groq または OpenAI 互換）※未設定でもフォールバック動作
chatbot.api-key=gsk_...
# chatbot.model=llama-3.1-8b-instant  # デフォルト
# chatbot.base-url=https://api.groq.com/openai/v1  # デフォルト

# SendGrid（パスワード再設定メール・請求通知メール）
sendgrid.api-key=SG....
sendgrid.from-email=noreply@petlife.local
app.base-url=http://localhost:8080

# 請求・振込先情報（BillingController / BillingNotificationService）
billing.bank-name=〇〇銀行
billing.bank-branch=△△支店
billing.bank-account-number=12345678
billing.bank-account-holder=PetLife 事務局
billing.paypay-qr-url=（PayPay 支払い QR URL）
billing.paypay-id=（PayPay ID）
```
...
| 診療記録 CRUD（SUPER / VET / STAFF） | `ConsultationController` |
...
| お知らせ管理（SUPER / ADMIN / STAFF） | `AnnouncementController` |
| 予約枠管理（SUPER / ADMIN / STAFF） | `AppointmentSlotController` |
