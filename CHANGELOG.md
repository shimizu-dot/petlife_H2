...
#### B-H3 — ローカル起動とバックアップを H2 前提へ変更
- **変更ファイル:**
  - `backend/pom.xml`
  - `backend/src/main/resources/application.properties`
  - `backend/src/main/resources/schema.sql`
  - `backend/src/main/resources/data.sql`
  - `backend/src/main/java/com/example/petlife/service/DatabaseBackupService.java`
  ...
  - `backend/docs/prompt_backend.md`
  - `backend/src/main/resources/templates/admin/database.html`
  - `backend/src/main/resources/META-INF/additional-spring-configuration-metadata.json`
  - `backend/src/test/java/com/example/petlife/PetlifeApplicationTests.java`
  - `CHANGELOG.md`
- **変更内容:**
  1. デフォルトデータソースを H2 の in-memory DB に切り替え、PostgreSQL ドライバ依存を外した
  2. スキーマ定義の `JSONB` / `BIGSERIAL` / 部分ユニークインデックスを H2 互換の定義へ置き換えた
  3. バックアップ・リストアを H2 の `SCRIPT` / `RUNSCRIPT` へ変更し、Docker Compose も DB コンテナ不要の構成に整理した

### ドキュメント更新（Low — README を新規作成）

#### D-L16 — リポジトリの起動手順と構成を README に整理
- **変更ファイル:**
  - `README.md`
  - `CHANGELOG.md`
- **変更内容:**
  1. プロジェクト概要、主要機能、構成を README に追加した
...
#### D-L11 — `docs/test_report_document.xlsx` に結果集計シートを追加し、`docs/09-test-report.html` のサマリーを更新
- **変更ファイル:**
  - `docs/test_report_document.xlsx`
  - `docs/09-test-report.html`
  - `CHANGELOG.md`
- **変更内容:**
  1. `テスト結果サマリー` シートを追加し、`テストケース一覧` の結果列を関数で集計するようにした
  2. `総テストケース数 / OK / NG / 未実施 / 合格率` を workbook 上で表示できるようにした
  3. `docs/09-test-report.html` の「3 テスト結果サマリー」を workbook 集計値に合わせて更新した

#### D-L12 — `機能別テスト項目` も集計対象に含めてサマリーを拡張
- **変更ファイル:**
  - `docs/test_report_document.xlsx`
  - `docs/09-test-report.html`
  - `CHANGELOG.md`
- **変更内容:**
  1. `機能別テスト項目` の結果列も集計対象に追加した
  2. workbook の `テスト結果サマリー` にシート別の内訳と合計値を表示するようにした
  3. `docs/09-test-report.html` のサマリーを合算値 `139 / 127 / 0 / 12 / 91.4%` に更新した

#### D-L13 — シート別合格率と合計合格率を個別表示へ変更
- **変更ファイル:**
  - `docs/test_report_document.xlsx`
  - `docs/09-test-report.html`
  - `CHANGELOG.md`
- **変更内容:**
  1. `テストケース一覧` と `機能別テスト項目` それぞれの合格率を分けて表示するようにした
  2. workbook の `テスト結果サマリー` で各シートの合格率と合計合格率を関数表示にした
  3. `docs/09-test-report.html` のサマリーを `92.7% / 90.5% / 91.4%` の構成に更新した

#### D-L14 — `テスト結果サマリー` に合格率比較グラフを追加
- **変更ファイル:**
  - `docs/test_report_document.xlsx`
  - `CHANGELOG.md`
- **変更内容:**
  1. `テストケース一覧` / `機能別テスト項目` / `合計` の合格率を比較するグラフを追加した
  2. workbook の `テスト結果サマリー` シート右側へグラフを挿入した
  3. グラフ用の集計セルをシート内に追加し、計算結果をそのまま参照できるようにした

#### D-L15 — グラフを Excel ネイティブのチャートとして明示的に調整
- **変更ファイル:**
  - `docs/test_report_document.xlsx`
  - `CHANGELOG.md`
- **変更内容:**
  1. グラフを画像ではなく Excel のチャートオブジェクトとして維持した
  2. データラベルを付けて、各合格率がシート上で直接読めるようにした
  3. 目盛り範囲を 0%〜100% にして比較しやすくした

## [2026-06-15]

### ドキュメント更新（Low — 機能別テスト項目を 4 区分で再整理）

#### D-L10 — `docs/test_report_document.xlsx` の機能別テスト項目を F-002 形式に統一
- **変更ファイル:**
  - `docs/test_report_document.xlsx`
  - `CHANGELOG.md`
- **変更内容:**
  1. `機能別テスト項目` シートを `F-002` と同じ 4 行 1 セットの構成にそろえた
  2. `正常系 / 異常系 / 境界値 / セキュリティ` ごとに `テスト手法・手順` を 1 行ずつ分離した
  3. 結果入力欄は右側に残し、各カテゴリの判定を個別に記録できるようにした

## [2026-06-12]

### ドキュメント更新（Low — 相談チャットボットの非対応注記を追加）

#### D-L9 — LINE AIチャットボット(β)の費用前提を反映して本アプリ内非対応へ修正
- **変更ファイル:**
  - `docs/test_report_document.xlsx`
  - `CHANGELOG.md`
- **変更内容:**
  1. `F-010` を本アプリ内では非対応である旨に修正した
  2. 関連テストケースを `SKIP` 前提に変更した
  3. テストマトリックスの説明文も非対応前提に合わせた

## [2026-06-12]

### ドキュメント更新（Low — テスト報告書の機能別テスト項目を具体化）

#### D-L8 — `docs/test_report_document.xlsx` の機能別テスト項目に手法・手順を追加
- **変更ファイル:**
  - `docs/test_report_document.xlsx`
  - `CHANGELOG.md`
- **変更内容:**
  1. `機能別テスト項目` シートに `テスト手法・手順` 列を追加した
  2. 異常系・境界値・セキュリティの具体的な確認手順を機能ごとに追記した
  3. 追加列に合わせて列幅とフィルタ範囲を更新した
